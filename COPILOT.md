...
src/                          — Extension backend (Node.js, VS Code API)
  ...
  extension.ts                — Entry: activate(), deactivate(), Chat Participant registration
  ...
  chatSessionTracker.ts       — Bridges Chat Participant events → agent state → webview
  copilotEventParser.ts       — Interprets Copilot tool events → webview messages
  copilotTools.ts             — Tool name registry, display formatting, animation categories
...
scripts/                      — 7-stage asset extraction pipeline
  ...
  3-vision-inspect.ts         — Vision auto-metadata
...
**Vocabulary**: Chat Session = Copilot Chat conversation. Agent = webview character bound 1:1 to a terminal + optional chat session. Terminal = VS Code terminal for user interaction.
...
**One-agent-per-session**: Each "+ Agent" click → new terminal (`Copilot Agent #N`) → immediate agent creation → Chat Participant monitors tool invocations.

**Strategy B (Chat Participant + LM Tools API)**: A `@pixel-agents` Chat Participant is registered in `extension.ts`. When a user interacts with it, the participant handler forwards requests to the language model and intercepts tool call/result parts from the response stream. These events flow through `ChatSessionTracker` → `copilotEventParser` → webview messages.

## Agent Status Tracking

Tool invocations are detected via the Chat Participant's response stream (`LanguageModelToolCallPart`, `LanguageModelToolResultPart`). Turn completion is detected when the response stream ends.

**Tool name mapping**: `copilotTools.ts` maps every Copilot built-in tool to a display string and animation category. Key tools: `read_file` → "Reading", `replace_string_in_file`/`multi_replace_string_in_file` → "Editing", `create_file` → "Writing", `run_in_terminal` → "Running", `grep_search`/`file_search`/`semantic_search` → "Searching", `fetch_webpage` → "Fetching", `runSubagent` → "Subtask", MCP tools → prefixed by provider name. Unknown tools → "Using <name>".

**Animation categories**: Tools map to either `typing` (writes, edits, terminal commands, tasks) or `reading` (reads, searches, listings, fetch). This determines the character's animation in the webview.

**Permission-exempt tools**: `runSubagent`, `manage_todo_list`, `ask_questions` — these don't trigger permission timers when running.

**Event flow**: Chat Participant → `ChatSessionTracker.handleToolEvent()` → `copilotEventParser.processCopilotToolEvent()` → `agentToolStart`/`agentToolDone` webview messages. Status changes → `processCopilotStatusChange()` → `agentStatus` messages.

**Extension state per agent**: `id, terminalRef, chatSessionUri?, activeToolIds, activeToolStatuses, activeToolNames, activeSubagentToolIds, activeSubagentToolNames, isWaiting, permissionSent, hadToolsInTurn`.
...
- **Turn-end detection**: Response stream completion signals turn end. All tool state cleared as safety measure. Text-idle timer (`TEXT_IDLE_DELAY_MS = 5s`) as fallback for edge cases where the stream doesn't complete cleanly.
...
- Chat Participant (`@pixel-agents`) intercepts requests and monitors tool invocations
- `LanguageModelToolCallPart` / `LanguageModelToolResultPart` in response stream = tool start/done
...
- Strategy B (Chat Participant + LM Tools API) chosen for Copilot integration — most stable API
