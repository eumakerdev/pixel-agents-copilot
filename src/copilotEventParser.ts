...
import { formatCopilotToolStatus, COPILOT_PERMISSION_EXEMPT_TOOLS } from './copilotTools.js';

/**
 * Process a Copilot tool invocation event (start or done).
 * Replaces the JSONL-based processTranscriptLine() from transcriptParser.ts.
 */
export function processCopilotToolEvent(
	...
	if (event.type === 'start') {
		...
		if (!COPILOT_PERMISSION_EXEMPT_TOOLS.has(toolName)) {
			startPermissionTimer(agentId, agents, permissionTimers, COPILOT_PERMISSION_EXEMPT_TOOLS, webview);
		}
...
export function processCopilotSubagentToolEvent(
	...
	if (event.type === 'start') {
		const toolName = event.toolName;
		const status = formatCopilotToolStatus(toolName, event.input || {});
		console.log(`[Pixel Agents] Agent ${agentId} subagent tool start: ${event.toolId} ${status} (parent: ${parentToolId})`);
		...
		console.log(`[Pixel Agents] Agent ${agentId} subagent tool done: ${event.toolId} (parent: ${parentToolId})`);
